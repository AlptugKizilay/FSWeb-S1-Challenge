...
function profilListesi(arr) {
  ...
  for(let i = 0; i<arr.length; i++){
    //console.log(tempArr[i].profile);
    names.push(tempArr[i].profile);
  }
  return names;
}
...
function fenomenSil(arr,indeks) {
  ...
  let removed = copyArr.splice(indeks,1);
  return copyArr;
}
...
function fenomenEkle(arr,number,profile,followers,posts,platform) {
  ...
  copArr.push(FenomenObject);
  return copArr;
}
console.log(fenomenEkle(fenomenler,6,"Workitech",10000000,2022,"Instagram"));
...
function enFenomenler(arr) {
  ...
  for(let i = 0; i<arr.length; i++){
    if(arr[i].followers > 100000000){
      highFollowers.push(arr[i].profile);
    }
  }
  return highFollowers;
}
...
function platformaGoreCokGonderiYapanFenomen(arr, platformName){
  ...
  for(let i = 0; i<arr.length; i++){
    if(arr[i].platform === platformName){
      if (arr[i].posts !== 'NA' && arr[i].posts > maxPost) {
        maxPost = arr[i].posts;
        maxPostFenomen = arr[i].profile;
      }

    }
  }
  return maxPostFenomen; 
}
